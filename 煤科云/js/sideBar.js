...
var cl = `<ul class="sidebar-menu">
			<li id="cp" class="nav-item start">
				...
				<li id="jjfa" class="nav-item start">
...
$.ajax({
	...
	success: function(data) {
		...
		var html = '';
		for(var i = 0; i < data.list.length; i++) {
			html += '<li><a href="javascript:;">' + data.list[i].name + '</a>';
			html += '<ul class="sub-menu sub-menu-three w-344">';
			$.ajax({
				cache: true,
				type: "get",
				...
				success: function(data) {
					for(var j = 0; j < data.list.length; j++) {
						html += '<li><a href="jiejuefangan-nav2.html">' + data.list[j].name + '</a></li>'
					}
				}
			})
			html += '</ul>';
